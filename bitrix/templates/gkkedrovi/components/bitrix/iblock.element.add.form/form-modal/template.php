	...
	<div class="row gy-3">
		...
		<? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])) : ?>
			...
			<? foreach ($arResult["PROPERTY_LIST"] as $propertyID) : ?>
				...
				?>
					<input class="form-control form__input  rounded-2 <?= $propertyID == 14 ? ' form-phone' : '' ?> p-3 h-100" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" value="<?= $value ?>" placeholder="" required>
					<label for="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" class="form-label form__label position-absolute top-50 fs-16 "><? if (intval($propertyID) > 0) : ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else : ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?></label>
					<?
		...
		<div class="">
			<button type="submit" class="btn form-btn fs-20 fw-500 p-3 w-100" name="iblock_submit" value="Отправить">
				Отправить
			</button>
		</div>
	</div>
